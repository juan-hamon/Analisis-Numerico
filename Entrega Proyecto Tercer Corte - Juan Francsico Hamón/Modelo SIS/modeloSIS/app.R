...
server <- function(input, output) {
    ...
    output$distPlot <- renderPlot({
        ...
        sol = ode(y=v_iniciales, times=dt, func=SIS,parms=parametros, method = input$tipoModelo)
        ...
        N <- sum(v_iniciales)
        plot(dt, S, type="l", col="blue", ylim=c(0,sum(v_iniciales)),xlab="Tiempo (en Horas)", ylab="Numero de individuos (en miles)")
        lines(dt, I, type="l", col="red")
        lines(dt, result$results[,2], type = "l", col = "brown")
        lines(dt, result$results[,3], type = "l", col = "orange")
        title("Modelo SIS:")
        legend((input$ValHoras)/2, N/1.5, legend=c("Susceptibles por ODE", "Infectados por ODE", "Suceptibles por EpiDynamics", "Infectados por EpiDynamics"), col=c("blue", "red", "brown", "orange"), lty=rep(1, 2)) 
    })
